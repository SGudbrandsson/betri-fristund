...
for their children. Loads pre-fetched event data from `events.json` (updated
daily by GitHub Actions) with live fallback to [frístund.is](https://fristund.is)
via CORS proxies.
...
betri-fristund/
  ...
  tests.js           # Test suite (node tests.js), ~173 tests
  .gitignore
  scripts/
    fetch-events.js  # Server-side fetch/filter/enrich script (no deps)
  .github/
    workflows/
      fetch-events.yml  # Daily scheduled workflow to refresh events
  docs/              # Served by GitHub Pages
    index.html       # Single-page HTML with data-i18n attributes
    app.js           # All application logic (~950 lines, vanilla JS IIFE)
    styles.css       # All styles (~830 lines, mobile-first, CSS variables)
    events.json      # Pre-fetched event data (generated by fetch-events.js)
```

## How It Works (User Flow)

1. User lands on welcome screen, selects child's age
2. App loads pre-fetched `events.json` (updated daily by GitHub Actions)
3. If JSON is missing or stale (>48h), falls back to live fetch via CORS proxies
4. Filters out non-activity items (fees, merchandise, subscriptions, adult-only)
5. Client-side filtering by user criteria (age, tags, location, dates)
6. Renders activity cards with images, dates, locations, descriptions, and signup links
...
1. **Configuration** — `BASE_URL`, `CORS_PROXIES`, `EVENTS_JSON`, `EVENTS_MAX_AGE_MS`
2. **TAG_CATEGORIES** — 23 categories with `label`, `labelEn`, `icon`, `tags`
3. **TAG_LABELS** — display name lookup for all known API tags
4. **TRANSLATIONS** — IS/EN strings (34 keys per language)
5. **POSTAL_NAMES** — postal code → neighborhood name map (16 entries)
6. **isValidActivity()** — exclusion filter (28 rules: regex + age + club checks)
...
13. **JSON-first loading** — `loadEventsJson()`, `filterCachedCards()`, `fetchFromJson()`
14. **Live fallback** — `buildUrl()`, `parseNextData()`, `fetchPage()`, `fetchFilteredLive()`
15. **API dispatcher** — `fetchFiltered()` (tries JSON first, falls back to live)
16. **UI rendering** — dropdowns, tag chips, cards, results info, load more
17. **Actions** — `search()`, `loadMore()`, `clearFilters()`
18. **Event binding** — `bindEvents()`
19. **Init** — `init()`

### JSON-First Data Loading

The app loads pre-fetched `events.json` by default (same-origin, no CORS issues):
1. `loadEventsJson()` — fetches and caches `events.json`, checks staleness (48h max)
2. `filterCachedCards()` — filters cached events by user criteria (age, tags, location, dates) client-side
3. `fetchFromJson()` — combines loading + filtering, returns paginated results
4. If JSON is unavailable or stale, `fetchFiltered()` falls back to live scraping

### CORS Proxy Chain (Live Fallback)

Direct fetch to fristund.is is blocked by CORS. The live fallback tries proxies in order:
...
### Smart Pagination (Live Fallback)

Heavy filtering (via `isValidActivity()`) can remove most results from a page.
`fetchFilteredLive()` loops through API pages until at least `MIN_RESULTS` (6)
valid cards are accumulated, or there are no more pages. This only runs when the
JSON-first path fails.

### GitHub Actions Pipeline (`scripts/fetch-events.js`)

A zero-dependency Node.js script runs daily via GitHub Actions:
1. **Fetch** — paginate through all listing pages on fristund.is (no CORS server-side)
2. **Filter** — apply `isValidActivity()` to remove non-kid items
3. **Enrich** — fetch each valid event's detail page (`/namskeid/{id}`) for description, full address, and signup URL
4. **Write** — save enriched events to `docs/events.json`

The workflow (`.github/workflows/fetch-events.yml`) runs at 06:00 UTC daily and auto-commits if data changed.
...
for any title/card matching these patterns:
...
| Participant fees | `/iðkendagjald\|árgjald/i` | "Iðkendagjald" |
| Membership/access | `/félagsaðild\|\baðild\b/i` | "Félagsaðild" |
| Adult-only events | `/\bfullorðn/i` (unless also `/börn/i`) | "Fullorðnir - Líkamsrækt" |
| Masters programs | `/\bmasters\b/i` | "Masters golf" |
| Senior/60+ programs | `/\b60\s*\+\|60 ára og eldri\|\beldri borgar/i` | "60+ Líkamsrækt" |
| International trips | `/\b(FIN\|CHE\|...\|SWE)\b/` + `/meistaramót\|bikarmót/i` | "EM FIN 2026" |

### Club/age-based rules

| Rule | Check | Example rejected |
|------|-------|-----------------|
| World Class adult fitness | `clubname` starts with "World Class" + adult class titles | "Hot Yoga" by World Class |
| Vesenisferðir hiking | `clubname` starts with "Vesenisferðir" | All hiking tours |
| Hilton SPA | `clubname` matches `/hilton.*spa/i` | Spa memberships |
| Age min ≥ 18 | `Math.min(...card.age) >= 18` | Adult-only (age `[126, 18]`) |
...
All tests must pass before pushing. Currently ~173 tests.
...
`tests.js`. This includes `isValidActivity()` which must be kept in sync across
`docs/app.js`, `tests.js`, and `scripts/fetch-events.js`.
...
- `isValidActivity()` — exclusion filter (~105 tests, including title, age, and club rules)
...
4. Copy the updated function to `scripts/fetch-events.js` (replace the old version)
5. Add test cases in `tests.js` under the `isValidActivity` section
6. Run `node tests.js`
...
The fetch script runs automatically via GitHub Actions daily at 06:00 UTC.
To run it manually:
...
This fetches all events from frístund.is, filters them, enriches with
descriptions/addresses/signup URLs, and writes to `docs/events.json`.
Runtime is ~5-7 minutes (rate-limited to be polite to the server).

To trigger the GitHub Action manually, use the "Run workflow" button in the
Actions tab, or:

```bash
gh workflow run fetch-events.yml
```
